...
class Kutu_Form_Helper_AuthorClinicInputGenerator
{
	function generateFormAdd($profileGuid, $folderGuid=null)
	{
		if(empty($folderGuid))
			throw new Zend_Exception('Kutu_Form_Helper_AuthorClinicInputGenerator: Can not generate form with empty folderGuid');
		...
		foreach ($rows as $row)
		{
			...
			$attributeRenderer = new Kutu_Form_Attribute_Renderer($row3->guid,null,$row3->type,null,null,'clinic_partner');
			$aRenderedAttributes[$row3->guid]['description'] = ($row3->description == 'Category')? 'Clinic partner' : $row3->description;
